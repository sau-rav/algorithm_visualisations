...
function addToObstacle(i, j) {
  if(visitedMap[floor(i / obstacleSize)][floor(j / obstacleSize)] == 2) return;
  obstacleMatrix.push([floor(i / obstacleSize) * obstacleSize, floor(j / obstacleSize) * obstacleSize]);
  visitedMap[floor(i / obstacleSize)][floor(j / obstacleSize)] = 2;
}
...
function setup() {
  ...
  noStroke();
  flag = -2;
  search = false; path_found = false; path_not_found = false;
  ptr = 0;
  obstacleMatrix = []; start = []; end = []; visited = []; queue = [];
...
function mouseClicked() {
  if (flag == 0) {
    start[0] = mouseX - mouseX % obstacleSize;
    start[1] = mouseY - mouseY % obstacleSize;
    if (!isObstacle(start) && mouseX >= 0 && mouseX <= size && mouseY >= 0 && mouseY <= size) {
      flag += 1;
    } else start = [];
  } else if (flag == 1) {
    end[0] = mouseX - mouseX % obstacleSize;
    end[1] = mouseY - mouseY % obstacleSize;
    ...
    if (mouseX >= size / 2 + 30 && mouseX <= size / 2 + 290 && mouseY >= size + 10 && mouseY <= size + 35) {
      flag = 0;
      customDrawing();
    }
    else if(mouseX >= size / 2 -293 && mouseX <= size / 2 - 53 && mouseY >= size + 10 && mouseY <= size + 35){
      flag = -1;
    }
  } else if (flag == -1) {
    if (mouseX >= size / 2 - 80 && mouseX <= size / 2 + 72 && mouseY >= size + 10 && mouseY <= size + 35) {
      flag = 0;
    }
  }
}
...
function startProcess() {
  flag = 3;
  queue.push(new Point(start[0], start[1], [
    [start[0], start[1]]
  ], 0, 0));
}
...
function draw() {
  ...
  if (flag == -2) {
    push();
    fill(200);
    rect(size / 2 + 30, size + 10, 260, 25);
    rect(size / 2 - 293, size + 10, 240, 25);
    fill(0);
    textSize(20);
    text("Draw custom obstacles", size / 2 - 290, size + 27);
    text("Use predefined obstacles", size / 2 + 33, size + 27);
    textSize(15);
    text("or", size / 2 - 25, size + 27);
    pop();
  }
  if(flag == -1) {
    push();
    fill(200);
    rect(size / 2 - 80, size + 10, 152, 25);
    fill(0);
    textSize(20);
    text("Finish Drawing", size / 2 - 77, size + 27);
    pop();
  }
  if (flag == 0) {
    push();
